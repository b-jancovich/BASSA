%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This modified version of "nfaslt.m" by Harald Barzan, is intended to be
% ported to C++ using the MATLAB Coder. 
% 
% Modifications by Ben Jancovich in March 2023
% MammalLab, School of Biological Earth and Environmental Sciences
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [wtresult] = nfaslt_CPP(input, Fs, Fi, Nf, c1, o, mult) %#codegen 
...
if (~isempty(o))
    ...
    if (Fi(1) > 0)
        ...
        if (Fi(2) < 0)
            ...
            for i = 1 : Nf
                order_frac(i) = r2rmap_CPP(abs(F(i)), 0, Fi(2), o(1), o(2));
            end
        end
    end  
...
for i_freq = 1 : numel(F)
    ...
    for i_ord = 1 : order_int(i_freq)
        ...
        wavelets{i_freq, i_ord} = cxmorlet_CPP(-F(i_freq), n_cyc, Fs);
...
for i_buf = 1 : Nbuffers
    for i_freq = 1 : numel(F)
        ...
        if (is_fractional_CPP(order_frac(i_freq)) && ...
